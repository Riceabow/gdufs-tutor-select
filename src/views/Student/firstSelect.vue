<template>
  <div class="" v-if="isStarted">
    <div>
      <h1>第一轮</h1>
      <h2>第一轮可选导师情况</h2>
      <h4 v-if="!isShow" style="color: brown">
        已完成导师选择或者该轮选择导师时间已过,无法进行选择
      </h4>
      <n-data-table
        :columns="columns1"
        :data="data1"
  ...
  >
    <div style="padding: 20px">
      <p>确认选择后则<b style="color: brown;">无法更改</b>，您确定要选择导师{{ selectedTutor.name }}吗？</p>
...
// console.log(isShow.value,'haveSelected',haveSelected.value,'isEnd',isEnded.value)
const data1 = ref([
  {
    name: "加载数据失败，网络问题",
    major: ["方向1", "方向2"],
...
const userInfo = useUserStore().userInfo.value
const myMajor = userInfo.major;  //todo
const res = ref();
const time = ref([]);
...
onMounted(async () => {
  ...
  const firstRoundTime = time.value[0];
  const now = new Date();
  console.log(firstRoundTime);

  // 将字符串形式的时间转换为 Date 对象
  const startTime = new Date(firstRoundTime.startTime);
  const endTime = new Date(firstRoundTime.endTime);
...
const confirmChoose = async () => {
  ...
  if (row.studentNumber >= row.quota) {
    ...
    if (res.value.code == "200") {
      Nmessage.success("选择导师成功");
      haveSelected.value = true;
      reload();
    } else {
      Nmessage.error("选择导师失败，请稍后再试");
      reload();
    }
    cancelModal();
  }
